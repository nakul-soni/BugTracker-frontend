          ...
          <Droppable key={col.id} droppableId={col.id}>
            {(provided, snapshot) => (
              <div 
                ...
                className={`flex-shrink-0 w-[260px] rounded-xl bg-zinc-50 dark:bg-[#0B0E14] p-3 border border-zinc-200 dark:border-zinc-800/80 flex flex-col ${snapshot.isDraggingOver ? 'bg-indigo-50/50 dark:bg-indigo-900/10 border-indigo-200 dark:border-indigo-800/50' : ''}`}
              >
                <h3 className="font-semibold text-xs uppercase tracking-wider mb-3 text-zinc-700 dark:text-zinc-300 flex items-center justify-between">
                  {col.title} <Badge variant="secondary" className="bg-zinc-200 dark:bg-zinc-800/80 text-[10px] px-2 py-0 border-none">{boardData[col.id]?.length || 0}</Badge>
                </h3>
                    ...
                    <Draggable key={bug.id} draggableId={bug.id} index={index} isDragDisabled={isDragDisabled(bug)}>
                      {(provided, snapshot) => (
                        <div
                          ...
                          className={`bg-white dark:bg-[#151923] p-3 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-800/80 transition-colors ${isDragDisabled(bug) ? 'cursor-not-allowed opacity-60 hover:bg-zinc-50 dark:hover:bg-zinc-800/30' : 'cursor-grab active:cursor-grabbing hover:border-indigo-400 dark:hover:border-indigo-500/50'} ${snapshot.isDragging ? 'shadow-2xl ring-2 ring-indigo-500 z-50' : ''}`}
                        >
                          <div className="flex justify-between items-center mb-2">
                            <span className="text-[10px] font-mono text-zinc-400 dark:text-zinc-500">BUG-{bug.id.slice(0,4)}</span>
                            <Badge className={`${severityColors[bug.severity]} border-none text-white text-[9px] px-1.5 py-0 leading-tight shadow-sm`}>{bug.severity}</Badge>
                          </div>
                          ...
                          <div className="flex items-center justify-between pt-2 border-t border-zinc-100 dark:border-zinc-800/50">
                            <span className="text-[10px] text-zinc-500 dark:text-zinc-400 truncate flex items-center gap-1.5">
                              <div className="w-4 h-4 rounded-full bg-indigo-100 dark:bg-indigo-500/10 flex items-center justify-center text-[8px] font-bold text-indigo-600 dark:text-indigo-400">
